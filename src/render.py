...
class blob(dict):
  ...
  def load_sensor(cls, r, phi, theta,resolution:tuple=(256,256)):
    ...
    ######################################################
def __repr__(cls):
    return f"< mitsume blob using {cls.integrator} integrator"

def __str__(cls):
    import pprint
    return pprint.pprint(cls)

#f"contains {cls.obj_count}-obj {cls.ball_count}-ball {} rect and {} cubes >"
# TODO: add setters, getters etc.
@property
def integrator(cls):
    return cls.integrator
@integrator.setter
def integrator(cls,val):
    if type(val) != str:
        print("should be a string but i trust you")
    cls.integrator = val
#self['integrator'] = {'type': integrator,'max_depth': max_depth}
